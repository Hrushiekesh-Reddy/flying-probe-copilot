...
**Discovered:** 2026-06-18
**Phase:** Phase 2 — flaky-test triage (branch `claude/beautiful-beaver-e50f90`).
**File(s):** `tests/test_parser/test_log_parser.py` (the `_render_to_text` helper + its two callers). Production code (`renderers/log.py`, `parser/log_parser.py`) was exonerated — no source change needed.
**Symptom:** `tests/test_parser/test_log_parser.py::test_tokenize_balances_braces_returns_records` passed when run in isolation but failed intermittently in the full suite (`uv run pytest`). Two failure faces: (a) `AssertionError` — `len(records) < 2 + len(boards)` (tokenizer saw a partially-written / truncated file); (b) on Windows, `PermissionError: [WinError 32] The process cannot access the file because it is being used by another process`.
**Root Cause:** The test-file helper `_render_to_text` rendered the `BatchLog` to a **single fixed path at the worktree root** — `Path(__file__).parent.parent.parent / "tmp_test_render.log"` — then read it and `unlink(missing_ok=True)`-ed it. That path is shared by the two tests that call the helper (`test_tokenize_balances_braces_returns_records` and `test_tokenize_returns_batch_and_btest_prefixes`). It was the ONLY test in the suite writing to a fixed path in the repo tree; every other test uses pytest's per-test `tmp_path` or `NamedTemporaryFile`. A file living in the repo working tree is actively watched by editors (VS Code file watcher), git, antivirus, and the Windows Search indexer, so during a busy full-suite run the freshly-created file gets transient locks → `read_bytes()` reads a partial write (→ assertion fail) or `unlink()`/`read` raises `PermissionError` (only `FileNotFoundError` was suppressed). Under any parallel/repeated execution (xdist, CI shards, or two concurrent runs in the same checkout) the two callers race the same path directly. The generator fixture itself is fully deterministic (`random.Random(seed)`, no `hash()` in its path), so test content was never the variable — only the shared file was.
**Fix:** Changed `_render_to_text(batch_log, encoding=...)` → `_render_to_text(batch_log, tmp_path, encoding=...)`: it now renders to `tmp_path / "render.log"` (pytest's per-test unique dir, under the OS temp dir, outside any repo-tree watcher) and returns the decoded bytes with no manual unlink (pytest cleans `tmp_path`). The two callers gained a `tmp_path` parameter. No production code changed.
