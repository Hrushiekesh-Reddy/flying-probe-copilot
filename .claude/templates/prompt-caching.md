...
> Generated by hrk-agent-starter / stamp.ps1. No customization needed — this is structural.
>
...
- Document step — write the brief
- Explore step — dispatches sub-agent #1
- Read result (≤1-2 min)
- Plan step — parent work
- Verify Plan (if Large) — dispatches sub-agent #2 with same brief
- Execute — dispatches sub-agent #3 with same brief
- Verify Exec (if Large) — dispatches sub-agent #4 with same brief

If you take a long break between Plan and Execute, that's fine for human reasons — but understand the brief prefix will cost full price on the next dispatch.
...
T+00:30  Parent writes session brief. [tiny incremental cost]
...
The brief block win shows up between Explore and Exec because they fired close together. Verify-Exec missed because we lingered. For tiered workflows, this is fine — you're trading some cache hits for the parent's careful Triple Check read.
...
You don't configure prompt caching directly. You enable it by being disciplined about prefix stability. The brief-block template + tier-batching strategy in this kit are designed to maximize cache hits across the sub-agents of a full loop. Expect ~30–50% input-token savings on Medium/Large sessions when followed strictly.
